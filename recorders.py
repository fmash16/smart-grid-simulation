...
object multi_recorder {
    ...
    interval 60;
    property LOAD%(number)d:base_power_%(phase)c, Bus%(bus)d:measured_power.real, inv_%(number)d_sol:VA_Out.real, inv_%(number)d_batt:VA_Out.real, Bus%(bus)d:measured_voltage_%(phase)c.real, Bus%(bus)d:measured_current_%(phase)c.real;
}
